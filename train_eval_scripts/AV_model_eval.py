...
# PATH
model_path = './saved_AV_models/AVmodel-2p-002-0.48038.h5'
dir_path = '/content/drive/MyDrive/predictions/'
if not os.path.isdir(dir_path):
    os.mkdir(dir_path)
database_path = '/content/drive/MyDrive/ASR/speech_separation-master/data/audio/AV_model_database/mix/'
face_path = '/content/drive/MyDrive/ASR/speech_separation-master/data/video/face_emb/'

# load data
testfiles = []
with open('/content/drive/MyDrive/AVdataset_train.txt', 'r') as f:
...
def parse_X_data(line,num_people=people_num,database_path=database_path,face_path=face_path):
    ...
    for i in range(num_people):
        # print(single_idxs[i])
        face_embs[0,:,:,:,i] = np.load(face_path+"%s_face_emb.npy"%single_idxs[i])
...
if NUM_GPU <= 1:
    for line in testfiles:
        print(line," out of ",len(testfiles))
        mix, single_idxs, face_embs = parse_X_data(line)
        mix_expand = np.expand_dims(mix, axis=0)
